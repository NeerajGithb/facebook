...
const LeftSideBar = () => {
  ...
  return (
    <aside
      ref={sidebarRef}
      className={`fixed top-[50px] left-0 h-full w-64 p-4 transform transition-transform duration-200 ease-in-out md:translate-x-0 flex flex-col z-50 md:z-0 ${
        isSidebarOpen
          ...
          <div className="flex items-center space-x-2 cursor-pointer "    onClick={() => handleNavigation(`/user-profile/${user?._id}`)}>
            <Avatar className="h-10 w-10">
              {user?.profilePicture ? (
                ...
                <AvatarFallback className="dark:bg-gray-200 bg-gray-300 dark:text-gray-800">
                  {userPlaceholder}
                </AvatarFallback>
              )}
            </Avatar>
          ...
          <div className="flex items-center space-x-2 mb-4 cursor-pointer "    onClick={() => handleNavigation(`/user-profile/${user?._id}`)}>
            <Avatar className="h-10 w-10">
              {user?.profilePicture ? (
                ...
                <AvatarFallback className="dark:bg-gray-200 dark:text-gray-800 bg-gray-300">
                  {userPlaceholder}
                </AvatarFallback>
              )}
            </Avatar>
