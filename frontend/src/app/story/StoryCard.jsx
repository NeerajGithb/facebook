...
const StoryCard = ({ isAddStory, story }) => {
  ...
  const { HandledeleteStory } = usePostStore()
  const [showPreview, setShowPreview] = useState(false);
  const [isNewStory, setIsNewStory] = useState(false);
  const isOwner = user?._id === story?.user?._id;
  const { fetchStoryPost } = usePostStore()
  ...
  const handleDeleteStoryPost = async () => {
    try {
      await HandledeleteStory(story._id); // Backend API call
      fetchStoryPost(); // Refresh story list
    } catch (error) {
    ...
    <>
              ...
              <div className="h-3/4 w-full relative border-b">
                <Avatar className="w-full h-full rounded-none">
                  {user?.profilePicture ? (
                    ...
                    <p className="w-full h-full flex justify-center dark:text-gray-300 items-center text-4xl">{userPlaceholder}</p>
                  )}
                </Avatar>
              </div>
              ...
              <div className="absolute top-2 left-2 ring-2 ring-blue-500 rounded-full ">
                <Avatar className="w-8 h-8">
                  {story?.user?.profilePicture ? (
                    ...
                    <AvatarFallback className="dark:text-gray-300">{userPlaceholder}</AvatarFallback>
                  )}
                </Avatar>
              </div>
      ...
      {showPreview && (
        <ShowStoryPreview
          ...
          onDelete={handleDeleteStoryPost} // Pass delete function
